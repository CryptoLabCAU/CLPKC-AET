// g++ -o MCL-PKC-AET MCL-PKC-AET.cpp -L. -lpbc -lgmp -lssl -lcrypto
// ./MCL-PKC-AET ../params/e256.param
/**MCL-PKC-AET.cpp**/

#include "../include/MCL-PKC-AET.h"
#include <iostream>
#include <cstring>
...
MCL_PKC_AET::MCL_PKC_AET(/*int lambda = 80*/)
{
    this->init();
}

MCL_PKC_AET::MCL_PKC_AET(int argc, char **argv)
{
    pbc_demo_pairing_init(pairing, argc, argv);
    this->init();
}

MCL_PKC_AET::~MCL_PKC_AET()
{
    this->clear();
}

void MCL_PKC_AET::init()
{
...
unsigned char *MCL_PKC_AET::Extract_Private_Value()
{
    element_t xID;
...
KEY *MCL_PKC_AET::Extract_Partial_Private_Key(const unsigned char *ID)
{
...
KEY *MCL_PKC_AET::Extract_Private_Key(const KEY dk, const unsigned char *x)
{
    KEY *sk = new KEY;
    element_t dk1, dk2;
    element_t sk1, sk2;
...
PK *MCL_PKC_AET::Extract_Public_Key(const unsigned char *x)
{
    PK *pk = new PK;
    element_t X, pk1, pk2;
    element_t xID;
...
CIPHER *MCL_PKC_AET::Encrypt(const unsigned char *ID, PK pk, const unsigned char *M)
{
    ...
    unsigned char *strC3_Left = new unsigned char[lenG1];
    unsigned char *strC3_Right = new unsigned char[SHA256_DIGEST_LENGTH];

    unsigned char *strC4_Left = new unsigned char[lenG1 + lenZr];
    unsigned char *strC4_Right = new unsigned char[lenGT + lenG1];
    unsigned char *strC4_Hash = new unsigned char[lenG1 + lenZr];
    ...
    memset(strC3_Left, 0x00, lenG1);

    memset(strC4_Left, 0x00, lenG1 + lenZr);
    memset(strC4_Right, 0x00, lenGT + lenG1);
    memset(strC4_Pair, 0x00, lenGT);
    memset(strC4_Hash, 0x00, lenG1 + lenZr);
    ...
    */
    ...
    memcpy(strC4_Right, strC4_Pair, lenGT);
    memcpy(strC4_Right + lenGT, strC3_Left, lenG1);
    H3(strC4_Right, lenGT + lenG1, strC4_Hash, lenG1 + lenZr);

    for (int i = 0; i < lenG1 + lenZr; i++)
        C->C4[i] = strC4_Left[i] ^ strC4_Hash[i];
...
unsigned char *MCL_PKC_AET::Decrypt(const KEY sk, const CIPHER C)
{
    element_t C1, C2, C3;
    element_t C3_Pair, C3_Left, C3_Right;
    element_t C4_Pair;
    element_t sk1, sk2;
    element_t verifyC1, verifyk;
    element_t m, r1, k;

    unsigned char *M = new unsigned char[lenG1];
    unsigned char *strR1 = new unsigned char[lenZr];
    unsigned char *strK = new unsigned char[lenG1];
    unsigned char *strC3_Pair = new unsigned char[lenGT];
    unsigned char *strC3_Right = new unsigned char[SHA256_DIGEST_LENGTH];
    ...
    element_init_G1(verifyk, pairing);
    element_init_G1(sk1, pairing);
    element_init_G1(sk2, pairing);
    ...
    element_div(k, C3, C3_Right);
    element_to_bytes(strK, k);

    memcpy(strC4_Right, strC4_Pair, lenGT);
    memcpy(strC4_Right + lenGT, strK, lenG1);

    H3(strC4_Right, lenGT + lenG1, strC4_Hash, lenG1 + lenZr);

    for (int i = 0; i < lenG1 + lenZr; i++)
    {
        strC4_Left[i] = C.C4[i] ^ strC4_Hash[i];
    }
    ...
    element_pow_zn(verifyk, m, r1);  // M^r1

    if (element_cmp(C1, verifyC1) || element_cmp(k, verifyk))
    {
...
unsigned char *MCL_PKC_AET::aut1(const KEY sk)
{
...
unsigned char *MCL_PKC_AET::aut2(const KEY sk, const CIPHER C)
{
...
unsigned char *MCL_PKC_AET::aut3i(const KEY sk, const CIPHER C)
{
    return aut2(sk, C);
}
unsigned char *MCL_PKC_AET::aut3j(const KEY sk)
{
    return aut1(sk);
}
unsigned char **MCL_PKC_AET::aut4(const KEY ski, const CIPHER Ci, const CIPHER Cj, element_t y)
{
    element_t td1, td2;
    element_t tmp1, tmp2;
    element_t pair;
...
bool MCL_PKC_AET::test1(const CIPHER Ci, const unsigned char *_tdi, const CIPHER Cj, const unsigned char *_tdj)
{
    bool flag = false;
    element_t Xi, Xj;
    element_t cmpi, cmpj;
...
bool MCL_PKC_AET::test2(const CIPHER Ci, const unsigned char *_tdi, const CIPHER Cj, const unsigned char *_tdj)
{
    bool flag = false;
    element_t Xi, Xj;
    element_t cmpi, cmpj;
...
bool MCL_PKC_AET::test3(const CIPHER Ci, const unsigned char *_tdi, const CIPHER Cj, const unsigned char *_tdj)
{
    bool flag = false;
    element_t Xi, Xj;
    element_t cmpi, cmpj;
...
bool MCL_PKC_AET::test4(const CIPHER Ci, const unsigned char **_tdi, const CIPHER Cj, const unsigned char **_tdj)
{
    bool flag = false;
    element_t Xi, Xj;
    element_t cmp1, cmp2;
...
pairing_t *MCL_PKC_AET::getPairing()
{
    return &this->pairing;
}

int MCL_PKC_AET::getLenG1()
{
    return this->lenG1;
}
int MCL_PKC_AET::getLenGT()
{
    return this->lenGT;
}
int MCL_PKC_AET::getLenZr()
{
    return this->lenZr;
}

void MCL_PKC_AET::H1(const unsigned char *src, const int slen, unsigned char *dest, int dlen)
{
    memset(dest, 0x00, dlen);

    SHA256(src, slen, dest);
}
void MCL_PKC_AET::H2(const unsigned char *src, const int slen, unsigned char *dest, int dlen)
{
    memset(dest, 0x00, dlen);

    SHA256(src, slen, dest);
}
void MCL_PKC_AET::H3(const unsigned char *src, const int slen, unsigned char *dest, int dlen)
{
    memset(dest, 0x00, dlen);
